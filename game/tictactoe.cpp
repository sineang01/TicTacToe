...
namespace Game
{
    ...
    const unsigned int TicTacToe::TIMER_AI_THINKING = 500;
    const unsigned int TicTacToe::TIMER_GAME_OVER = 2500;
    ...
    unsigned int TicTacToe::consoleBoardRight() const
    {
        return BOARD_OFFSET_LEFT + ((BOARD_CELL_WIDTH + 1) * mGameParams.boardWidth) + 1;
    }

    unsigned int TicTacToe::consoleBoardBottom() const
    {
        return BOARD_OFFSET_TOP + (BOARD_CELL_HEIGHT * mGameParams.boardHeight);
    }
