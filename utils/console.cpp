...
namespace Utils
{
    ...
    bool Console::write(char character, const Utils::PointUInt & coordinate)
    {
    ...
    bool Console::write(const std::string & text, const Utils::PointUInt & coordinate, bool clearLine)
    {
        DWORD charsWritten = 0;
        COORD coord = { static_cast<short>(coordinate.x()), static_cast<short>(coordinate.y()) };
    ...
    bool Console::clear(const Utils::PointUInt & coordinate, unsigned int length)
    {
        DWORD charsWritten = 0;
        COORD coord = { static_cast<short>(coordinate.x()), static_cast<short>(coordinate.y()) };
    ...
    bool Console::cursorAt(const Utils::PointUInt & coordinate)
    {
        COORD coord = { static_cast<short>(coordinate.x()), static_cast<short>(coordinate.y()) };
        return SetConsoleCursorPosition(mHandle, coord) != 0;
    }

    Utils::PointUInt Console::size() const
    {
